...
blargg_err_t YM2612_Impl::set_rate( long Rate, double clock_factor )
{
	require( Rate );
	require( clock_factor > 1.0 );
	...
	double Frequence = clock_factor / 144.0;
...
blargg_err_t YM2612_Emu::set_rate( long rate, double clock_factor )
{
	if ( !impl )
	{
		BLARGG_CHECK_ALLOC( impl = BLARGG_NEW YM2612_Impl );
		impl->mute_mask = 0;
	}
	memset( &impl->YM2612, 0, sizeof impl->YM2612 );
	return impl->set_rate( rate, clock_factor );
}
...
void YM2612_Emu::run_timer( int length ) { impl->run_timer( length ); }
...
void YM2612_Emu::run( sample_t* buf, int length ) { impl->run( buf, length ); }
