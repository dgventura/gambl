...
blargg_err_t Gym_Emu::init( long sample_rate, double gain, double oversample_ )
{
	...
	dac_synth.volume( 0.25 / 256 * gain );
	...
	long fm_sample_rate = sample_rate * oversample;
	double clock_factor = base_clock / 7.0 / fm_sample_rate;
	BLARGG_RETURN_ERR( fm.set_rate( fm_sample_rate, clock_factor ) );
...
blargg_err_t Gym_Emu::load( Emu_Reader& in )
{
	header_t h;
...
blargg_err_t Gym_Emu::load( const header_t& h, Emu_Reader& in )
{
	...
	mem = BLARGG_NEW byte [file_size];
	BLARGG_CHECK_ALLOC( mem );
...
void Gym_Emu::play_frame( sample_t* out )
{
	...
	blip_time_t clock_count = blip_buf.count_clocks( pairs_per_frame );
...
void Gym_Emu::parse_frame()
{
	...
	while ( (cmd = *pos++) != 0 )
	{
		...
		if ( cmd == 1 )
		{
			...
			else if ( dac_count < sizeof dac_buf )
			{
		...
		{
			//log_error(); // to do: many GYM streams are full of errors
			--pos; // put data back
		}
	}
	// loop
