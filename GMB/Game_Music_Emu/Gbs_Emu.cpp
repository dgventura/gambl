...
void Gbs_Emu::set_bank( int n )
{
	...
	if ( n == 0 && bank_count > 1 )
	{
		// to do: what is the correct behavior? Wario Land 3 and Tetris DX GBS
		// requires that this have no effect or set to bank 1.
		//return;
...
blargg_err_t Gbs_Emu::load( Emu_Reader& in )
{
	header_t h;
...
blargg_err_t Gbs_Emu::load( const header_t& h, Emu_Reader& in )
{
	...
	rom = BLARGG_NEW BOOST::uint8_t [rom_size];
	BLARGG_CHECK_ALLOC( rom );
