...
void ConvertCStringToUnicode( const char* inCString, HFSUniStr255* outUniName )
{
    outUniName->length = 0;
    CFStringRef cfname = CFStringCreateWithCString(kCFAllocatorDefault, inCString, CFStringGetSystemEncoding());
    if (cfname != NULL)
    {
        CFIndex cfnamelength = CFStringGetLength(cfname);
        if (cfnamelength > 255)
            cfnamelength = 255;
        outUniName->length = CFStringGetBytes(cfname, CFRangeMake(0, cfnamelength), kCFStringEncodingUnicode, 0, false, (UInt8*)(outUniName->unicode), 255, NULL);
        CFRelease(cfname);
    }
}
