...
public:
	...
	// Not very polished. Input and output is in stereo sample pairs.
	
	// Set size of buffer. Return true if out of memory.
	blargg_err_t buffer_size( int );
	
	// Set input/output resampling ratio and frequency rolloff. Returns
	...
	// Resample and output at most 'count' samples into 'buf', then remove input
	// samples from buffer. Return number of samples written into 'buf'.
